...
# Generate a single predictor variable (X) from a log-normal distribution
X <- rlnorm(n, meanlog = 1, sdlog = 0.2)
...
Y_true <- exp(beta0_true + beta1_true * log(X)) * epsilon

# Store in a data frame
data <- data.frame(X, Y_true)

### Step 2: Fit a Deterministic Linear Regression Model ###
lm_model <- lm(log(Y_true) ~ log(X), data = data)  
...
for (i in 1:n_sim) {
  ...
  X_sim <- rlnorm(n, meanlog = log(X), sdlog = 0.1)  
  ...
  Y_sim <- exp(beta0_sim + beta1_sim * log(X_sim))  
...
for (i in 1:num_realizations) {
  ...
  ensemble_data <- rbind(ensemble_data, data.frame(
    X = data$X,        # Keeping the same X values
...
plot2 <- ggplot() +
  # Plot all ensemble realizations as thin grey lines
  geom_line(data = ensemble_data, aes(x = log(X), y = log(Y_sim), group = realization),
            color = "grey", alpha = 0.3, size = 0.3) +
  # Scatter plot of true vs ensemble mean
  geom_point(data = data, aes(x = log(X), y = log(Y_mean_sim)), color = "red", alpha = 0.5, size = 2) +
  # Best-fit line for ensemble mean
  geom_smooth(data = data, aes(x = log(X), y = log(Y_mean_sim)), 
              method = "lm", color = "red", se = FALSE, size = 1.2) +
  # Blue uncertainty ribbon
  geom_ribbon(data = data, aes(x = log(X), ymin = log(Y_lower), ymax = log(Y_upper)), 
              fill = "blue", alpha = 0.2) +
  # Labels and theme
  labs(
    title = "Stochastic Ensemble Model",
    x = "Log(X)", y = "Log(Ensemble Mean Y)",
    subtitle = sprintf("R² Stochastic: %.3f", R2_stochastic)
  ) +
  theme_minimal()

# Plot 3: Comparison of Deterministic vs. Stochastic Predictions
plot3 <- ggplot(data, aes(x = log(X))) +
